...
Mesh RandomGreedy::ExtrudeImage(const Image& image) {
    Mesh mesh{ };
    ...
    while (pixelCount > 0) {
        int randX = static_cast<int>(RenderingUtilities::RandomFloat() * image.size.x);
        int randY = static_cast<int>(RenderingUtilities::RandomFloat() * image.size.y);

        glm::ivec2 startPoint{ randX, randY };
        glm::ivec2 endPoint{ startPoint };

        bool canExtendPosY = true;
        bool canExtendNegY = true;
        bool canExtendPosX = true;
        bool canExtendNegX = true;
        while (canExtendPosY || canExtendNegY || canExtendPosX || canExtendNegX) {
            // +Y
            if (endPoint.y + 1 < image.size.y) {
                canExtendPosY = true;
